...
df<- read.csv("apparent_optadjust_val_LUND_CORRECTED.csv") 

df<- df[df$Model_name != "Membership-based weighted model (weights limited to 1)" & df$Model_name != "Naive Logistic-developed on full data",]
df
...
for(i in 1:4){
  ...
  if(i==1){
    figs[[i]] <- ggplot(data, aes(y=Model_name, x=value, colour=variable, shape=variable))  + geom_point(size=2) +theme(
        ...
        axis.text.y=element_text(size=7), axis.title.y=element_blank(), axis.title.x=element_text(size=8.5),
      ...
      ) + labs(x = labels[i]) #+ guides(shape = guide_legend(nrow = 1, ncol=2))
  }
  
  if(i==2){
    figs[[i]] <- ggplot(data, aes(y=Model_name, x=value, colour=variable, shape=variable)) +
      geom_vline(xintercept = 0, linetype = "dashed", color = "black", size = 0.25) + geom_point(size=2 )  +theme(
      legend.position='none'  , axis.ticks.y = element_blank(), axis.text.y =element_blank(), 
        axis.title.y=element_blank(),axis.title.x=element_text(size=8.5),
      panel.background = element_rect(fill = "white", color = "gray"),  # White background with black borders
      panel.grid.major = element_line(color = "gray", size = 0.07),  # Light gray grid lines
      panel.grid.minor = element_line(color = "gray", size = 0.07),  # Minor grid lines
      strip.background = element_blank() 
...
filename<- "V2_adjusted_unadjusted_results.pdf"

pdf(filename, width =8, height = 3)
combined_plot <- annotate_figure(ggarrange(figs[[1]],figs[[2]], figs[[3]],  figs[[4]], 
                              ncol = 4, nrow = 1, widths=c(1.95,0.95,0.95,0.95), legend='none'))
