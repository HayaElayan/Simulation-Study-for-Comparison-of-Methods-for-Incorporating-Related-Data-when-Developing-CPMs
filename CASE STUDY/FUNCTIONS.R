validation_results <- function(model, validation){
  ...
  val_cstat_model <- roc(outcome ~ pr_val,data=validation)
  val_results[1,1] <- val_cstat_model$auc
  
  brierScore <- mean((pr_val- validation$outcome)^2)
  val_results[1,4] <- brierScore
  
  val_citl_model <- glm(outcome ~ offset(lp_val),family=binomial, data=validation)
  val_results[1,2] <- summary(val_citl_model)$coefficients[1,1]
  
  val_cslope_model <- glm(outcome ~ lp_val,family=binomial(link='logit'), data=validation)
...
propensity_weighting_limit1<- function(source, target){
  ...
  weight_formula <- as.formula( "isSource ~ Age+Sex+AF_atrial_flutter+Diabetes+BMI+LVEF35+eGFR")
...
weighted_LR <- function(source, target, ps_weights) {
  ...
  weight_formula <- as.formula( "outcome ~ Age+Sex+AF_atrial_flutter+Diabetes+BMI+LVEF35+eGFR+ isSource")
...
intercept_calibration <- function(source, target, dev_on, d_formula) {
  ...
  calibrated_model <- glm(outcome~offset(lp), data=target, family='binomial',x=T, y=T)#update intercept only
...
logistic_calibration <-function(source, target, dev_on, d_formula) {
  ...
  target$lp <- predict(source_model, newdata=target) #get Lp of source model on target data
  calibrated_model <- glm(outcome~lp, data=target, family='binomial',x=T, y=T)#update all model's coeff
...
all_data_model <- function(source, target){
  source$isSource <- 1
  target$isSource <- 0
  train_data <- rbind(source, target)
  
  dev_formula <- as.formula( "outcome ~ Age+Sex+AF_atrial_flutter+Diabetes+BMI+LVEF35+eGFR+ isSource")
...
manual_boot <- function(source, target,samples, dev_formula){
  ...
  for (i in 1:samples) {
    ...
    model_on_allData <- all_data_model(bs_samp_source, bs_samp_target)  
